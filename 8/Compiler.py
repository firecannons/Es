...
class CompilerIssue :
    ...
    @staticmethod
    def OutputError ( ErrorText ) :
        print ( FAIL_COLOR + ERROR_START_TEXT + ErrorText + END_COLOR )
    ...
    def ParseToken ( self , Token , SavedWordArray , WordIndex ) :
        if self . State == self . MAIN_STATES [ 'START_OF_LINE' ] :
            ...
            if Token . isalnum ( ) == False :
                CompilerIssue . OutputError ( Token + ' is not alphanumeric' )
                exit ( )
            else :
            ...
            if Token == self . SPECIAL_CHARS [ 'PERIOD' ] :
                ...
                CompilerIssue . OutputError ( Token + ' is not a period or newline' )
        return SavedWordArray
