...
entry ProgramStart'''
    ...
    def DoLetterIsWhiteSpaceInSetup ( self ,
        CurrentLetter ) :
        if self . DeclaringActionProgress == self . DECLARING_ACTION_STATES [ 'NONE' ] :
            self . PossiblyStartActionDeclaration ( )
        elif self . DeclaringActionProgress == self . DECLARING_ACTION_STATES [ 'WAITING_FOR_NAME' ] and self . IsSavedWordExisting ( ) == True :
            self . SavedFunctionItem . SetName ( self . SavedWord )
            self . DeclaringActionProgress = self . DECLARING_ACTION_STATES [ 'WAITING_FOR_OPENING_PAREN' ]
        elif self . DeclaringActionProgress == self . DECLARING_ACTION_STATES [ 'WAITING_FOR_PARAM_START' ] and self . IsSavedWordExisting ( ) == True :
            self . SavedParameterType = self . SavedWord
            self . DeclaringActionProgress = self .DECLARING_ACTION_STATES [ 'WAITING_FOR_PARAM_NAME' ]
        elif self . DeclaringActionProgress == self .DECLARING_ACTION_STATES [ 'WAITING_FOR_PARAM_NAME' ]  and self . IsSavedWordExisting ( ) == True : 
            NewFunctionParameter = FunctionItemParameter ( )
            NewFunctionParameter . SetType ( self . SavedParameterType )
            NewFunctionParameter . SetName ( self . SavedWord )
            self . SavedFunctionItem . AddFunctionItemParameter ( NewFunctionParameter )
            self . DeclaringActionProgress = self . DECLARING_ACTION_STATES [ 'WAITING_FOR_COMMA' ]
    ...
    def DoLetterIsMeaningfulInSetup ( self ,
        ...
        if self . DeclaringActionProgress == self . DECLARING_ACTION_STATES [ 'WAITING_FOR_OPENING_PAREN' ] :
            if CurrentLetter == self . OPENING_PAREN :
                self . DeclaringActionProgress = self . DECLARING_ACTION_STATES [ 'WAITING_FOR_PARAM_START' ]
            elif CurrentLetter == self . COMMA :
                self . DeclaringActionProgress = self . DECLARING_ACTION_STATES [ 'NONE' ]
        elif self . DeclaringActionProgress == self . DECLARING_ACTION_STATES [ 'WAITING_FOR_COMMA' ] :
            if CurrentLetter == self . CLOSING_PAREN :
                self . FunctionTable . append ( self . SavedFunctionItem )
                self . DeclaringActionProgress = self . DECLARING_ACTION_STATES [ 'NONE' ]
            elif CurrentLetter == self . COMMA :
                self . DeclaringActionProgress = self . DECLARING_ACTION_STATES [ 'WAITING_FOR_PARAM_START' ]
    ...
    def ProgramTranslation ( self , InputText ) :
        ...
        for i in self . FunctionTable :
            print ( i . Name )
            for i2 in i . Parameters :
                print ( '\t' , i2.Type , i2.Name )
        return ''
        '''
        NewText = ""
        Position = 0
