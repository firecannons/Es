...
class Parser ( ) :
    ...
    def Parse ( self , SavedWordArray ) :
        ...
        while WordIndex < len ( SavedWordArray ) :
            ...
            SavedWordArray , WordIndex = self . ParseToken ( Token , SavedWordArray , WordIndex )
            WordIndex = WordIndex + 1
        PrintObject ( self . TypeTable )
        return '' , SavedWordArray
    
    #def Reduce ( Token , SavedWordArray , WordIndex )
        #if SavedWordArray [ WordIndex ] 
    ...
    def ParseToken ( self , Token , SavedWordArray , WordIndex ) :
        ...
        if self . State == self . MAIN_STATES [ 'START_OF_LINE' ] :
            ...
            if Token == self . KEYWORDS [ 'USING' ] :
                ...
                self . Reduce ( Token , SavedWordArray , WordIndex )
                '''if self . CurrentFunctionType == self . FUNCTION_TYPES [ 'NORMAL' ] :
                    if Token in self . TypeTable :
                        ...
                        Found , ASymbol = self . CheckCurrentSTs ( Token )
                        
                        self . SavedLeftOperand = ASymbol
    ...
    def CheckCurrentSTs ( self , SymbolName ) :
        ...
        while Found == False and StackHeight - StackDepth >= 0 :
            if SymbolName in self . STStack [ StackHeight - StackDepth ] :
                Found = True
                OutSymbol = STStack [ StackHeight - StackDepth ]
...
class Lexer ( ) :
    ...
    def ProcessAppendWord ( self , Text ) :
        ...
        if self . State == self . MAIN_STATES [ 'NORMAL' ] :
            ...
            if Text == self . MAIN_STATES [ 'MULTI_LINE_COMMENT_END' ] :
                self . State = self . MAIN_STATES [ 'NORMAL' ]
                self . AppendWordToSavedArray ( Text )
        elif self . State == self . MAIN_STATES [ 'IN_STRING_LITERAL' ] :
            if Text [ len ( Text ) - 1 ] in self . QUOTATIONS :
    ...
    def DoLetterIsMeaningfulInSetup ( self ,
        ...
        if self . IsSavedWordExisting ( ) == True :
            self . ProcessAppendWord ( self . SavedWord )
        self . ProcessAppendWord ( CurrentLetter )
